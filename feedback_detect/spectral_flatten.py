...
class FeedbackGainRider:
    ...
    detected, then slowly returns to unity once the notch bank has engaged.

    Signal chain: ... → NotchBank → FeedbackGainRider → output

    On detection  → slam to DUCK_DB immediately (breaks the loop)
    While quiet   → ramp back to 0 dB at RELEASE_DB_PER_FRAME
    Re-detection  → slam back down, notch hasn't held yet
    """

    DUCK_DB             = -6.0    # how far to pull down on detection
    RELEASE_DB_PER_FRAME = 0.01   # +1 dB/s at 100 fps — slow return to unity
    MIN_DUCK_DB         = -12.0   # floor if rings keep firing
    ...
    def update(self, n_detections: int) -> float:
        """
        n_detections: len(detected_freqs) this frame (ALL detections, including re-triggers).

"""
voice_restore/eval_v3.py — Listen test for VoiceRestorer V3.

Usage:
    python voice_restore/eval_v3.py --input path/to/vocal.wav
    python voice_restore/eval_v3.py --input path/to/vocal.wav \
...
CKPT = PROJECT_ROOT / "checkpoints" / "voice_restore_v3" / "best.pt"
...
def run_eval(input_path: str, notch_specs: list[tuple], ckpt_path: Path, out_dir: Path):
    ...
    assert ckpt_path.exists(), f"No checkpoint at {ckpt_path} — train v3 first."
    ckpt = torch.load(str(ckpt_path), map_location=device)
    state = ckpt["model"] if isinstance(ckpt, dict) and "model" in ckpt else ckpt
    model = VoiceRestorerV3().to(device).eval()
    model.load_state_dict(state)
    print(f"Loaded VoiceRestorerV3 from {ckpt_path}")
    ...
    spectral, cond = make_v3_inputs(notched_mag, mask_db_t, f0_np, conf_np)
    ...
    print("  eval_restored.wav — after VoiceRestorer V3")
...
def main():
    ...
    ap.add_argument("--out-dir", default=str(PROJECT_ROOT / "data" / "eval_restore_v3"))
    args = ap.parse_args()

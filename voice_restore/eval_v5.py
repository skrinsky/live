"""
voice_restore/eval_v5.py — Listen test for VoiceRestorer V5.
"""
...
def run_eval(input_path: str, notch_specs: list[tuple], ckpt_path: Path, out_dir: Path, residual_floor: float):
    ...
    assert ckpt_path.exists(), f"No checkpoint at {ckpt_path} — train v5 first."
    ckpt = torch.load(str(ckpt_path), map_location=device)
    state = ckpt["model"] if isinstance(ckpt, dict) and "model" in ckpt else ckpt
    model = VoiceRestorerV5().to(device).eval()
    model.load_state_dict(state)
    print(f"Loaded VoiceRestorerV5 from {ckpt_path}")
    ...
    print(
        f"Raw residual: min={float(raw_residual.min()):.3f}  max={float(raw_residual.max()):.3f}  "
        f"mean={float(raw_residual.mean()):.3f}"
    )
    print(
        f"Base gain: min={float(base_gain.min()):.3f}  max={float(base_gain.max()):.3f}  "
        f"mean={float(base_gain.mean()):.3f}"
    )
    print(
        f"Effective gain: min={float(eff_gain.min()):.3f}  max={float(eff_gain.max()):.3f}  "
        f"mean={float(eff_gain.mean()):.3f}  | pct_bins_boosted={pct_boosted:.2f}%"
    )
    ...
    print("  eval_restored.wav — after VoiceRestorer V5")
